...
do
    ...
    local function make_mock(name)
        ...
        mt.__call = function(self, ...)
            ...
            if path:match("GetService$") then local s=args[2] or args[1]; if type(s)=="string" then return setmetatable({__path=s},mt) end end
            if path:match("FindFirstChild$") or path:match("WaitForChild$") then local c=args[2] or args[1]; if type(c)=="string" then return setmetatable({__path=path:gsub("%.[^.]+$","").."."..c},mt) end end
    ...
    Color3=make_mock("Color3"); BrickColor=make_mock("BrickColor"); TweenInfo=make_mock("TweenInfo"); Ray=make_mock("Ray")
    Region3=make_mock("Region3"); NumberRange=make_mock("NumberRange"); Drawing=make_mock("Drawing"); syn=make_mock("syn")
    ...
    checkcaller=function() return true end; isexecutorclosure=function(f) return type(f)=="function" end
    cloneref=function(x) return x end; gethui=function() return make_mock("HiddenUI") end
    ...
    firesignal=function() end; fireproximityprompt=function() end; fireclickdetector=function() end
    firetouchinterest=function() end; isnetworkowner=function() return true end
...
print('\n=== BUFFER ANALYSIS ===')
buf_count = int(runtime.lua.eval('#_G._all_buffers'))
print(f'Buffers created: {buf_count}')

for bi in range(1, buf_count + 1):
    size = int(runtime.lua.eval(f'_G._all_buffers[{bi}]._size'))
    bid = int(runtime.lua.eval(f'_G._all_buffers[{bi}]._id'))
    print(f'\nBuffer #{bid}: size={size:,} bytes')
    
    if size > 10_000_000:
        print('  Too large, skipping full dump')
        continue
    
    data = bytearray()
    chunk = 2048
    for off in range(0, size, chunk):
        end_off = min(off + chunk, size)
        lua_code = f'(function() local b=_G._all_buffers[{bi}]; local t={{}}; for i={off+1},{end_off} do t[#t+1]=string.char(b._data[i] or 0) end; return table.concat(t) end)()'
        chunk_data = runtime.lua.eval(lua_code)
        if isinstance(chunk_data, bytes):
            data.extend(chunk_data)
        else:
            data.extend(chunk_data.encode('latin-1'))
    
    fname = f'unobfuscator/cracked script/buffer_{bid}_{size}.bin'
...
print('\n=== ERRORS ===')
err_count = int(runtime.lua.eval('#_G._error_log'))
for i in range(1, min(err_count+1, 5)):
    print(f'  {runtime.lua.eval(f"_G._error_log[{i}]")}')
