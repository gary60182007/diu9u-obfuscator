...
# Split into lines for traceback
...
lines[10] = '\n'.join(parts)
new_source = '\n'.join(lines)
print(f"Split into {len(parts)} parts, {new_source.count(chr(10))} lines total")
...
# Use xpcall with message handler that captures full traceback
# Don't override error() - let errors propagate naturally
wrapper = """
_G._first_error = nil
_G._first_tb = nil
local function handler(err)
    if not _G._first_error then
        _G._first_error = tostring(err)
        _G._first_tb = debug.traceback(tostring(err), 2)
    end
    return err
end
...
if not ok then
    _G._final_error = tostring(result)
end
"""

print('Executing with xpcall handler (no error suppression)...')
t0 = time.time()
try:
    runtime.execute(wrapper)
    print(f'Completed in {time.time()-t0:.1f}s')
except LuaRuntimeError as e:
    print(f'Error after {time.time()-t0:.1f}s: {str(e)[:300]}')

first_err = runtime.lua.globals()['_first_error']
first_tb = runtime.lua.globals()['_first_tb']
final_err = runtime.lua.globals()['_final_error']

print(f'\n=== FIRST ERROR ===')
print(str(first_err)[:200] if first_err else 'None')

print(f'\n=== FIRST TRACEBACK ===')
print(str(first_tb)[:1000] if first_tb else 'None')

print(f'\n=== FINAL ERROR ===')
print(str(final_err)[:200] if final_err else 'None')

# Map line numbers
if first_tb:
    tb_str = str(first_tb)
    line_nums = set(re.findall(r'\]:(\d+):', tb_str))
    print(f'\n=== LINE MAPPING ===')
    for ln_str in sorted(line_nums, key=int):
        ln = int(ln_str)
        # The wrapper adds lines before the source
        # wrapper prefix has 7 lines, then new_source starts
        wrapper_prefix_lines = 7
        src_line = ln - wrapper_prefix_lines
        if 10 <= src_line < 10 + len(parts) + 10:
            part_idx = src_line - 11  # line 11 is where parts start (0-indexed from original line 10)
            if 0 <= part_idx < len(parts):
                code = parts[part_idx].strip()
                if len(code) > 300:
                    code = code[:300] + '...'
                print(f"  Line {ln} (src:{src_line}) → part[{part_idx}]: {code}")
