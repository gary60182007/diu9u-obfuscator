...
from .vm_lift import VMLifter, VMPrototype, VMInstruction, VMConstant, OpcodeMapping
...
class PSUChunk:
    ...
    sub_protos: List[PSUChunk] = field(default_factory=list)
    num_params: int = 0
    num_upvalues: int = 0
    max_stack: int = 0


class PSUDeobfuscator:
    ...
    def _is_psu(self, tree: ast.Block) -> bool:
        score = 0
        source = emit(tree)

        for marker in ['PSU', 'psu_', 'Prometheus']:
            if marker in source:
                score += 2
        ...
        for node in _walk(tree):
            if isinstance(node, ast.Call) and isinstance(node.func, ast.FunctionExpr):
                body = node.func.body
                if self._has_psu_patterns(body):
                    score += 3
            if isinstance(node, ast.While):
                for stmt in node.body.stmts:
                    if isinstance(stmt, ast.If) and len(stmt.tests) >= 8:
                        score += 2
            if isinstance(node, ast.LocalAssign):
                for val in node.values:
                    if isinstance(val, ast.String) and len(val.value) > 500:
                        score += 1

        return score >= 3

    def _has_psu_patterns(self, body: ast.Block) -> bool:
        has_xor = False
        has_byte_read = False
        has_dispatch = False
        ...
        return has_xor and has_byte_read and has_dispatch

    def _extract_chunk(self, tree: ast.Block) -> Optional[PSUChunk]:
        chunk = PSUChunk()
                ...
                for stmt in body.stmts:
                    if isinstance(stmt, ast.LocalAssign):
                        for val in stmt.values:
                            if isinstance(val, ast.FunctionExpr) and self._is_deserializer(val):
                                chunk.deserializer = val
                            if isinstance(val, ast.FunctionExpr) and self._is_vm(val):
                                chunk.vm_body = val.body

            if isinstance(node, ast.While):
                for stmt in node.body.stmts:
                    if isinstance(stmt, ast.If) and len(stmt.tests) >= 5:
                        chunk.vm_body = node.body

        if chunk.bytecode_data and chunk.deserializer:
            self._try_deserialize(chunk)
    ...
    def _is_vm(self, func: ast.FunctionExpr) -> bool:
        for node in _walk(func.body):
            if isinstance(node, ast.While):
                for stmt in node.body.stmts:
                    if isinstance(stmt, ast.If) and len(stmt.tests) >= 5:
    ...
    def _try_deserialize(self, chunk: PSUChunk):
        if chunk.deserializer is None or chunk.bytecode_data is None:
            return
        try:
            deser_src = emit(chunk.deserializer)
            self.sandbox.execute(f"local d = {deser_src}")
        except (LuaError, Exception):
            pass

    def _recover_opcodes(self, chunk: PSUChunk):
        if chunk.vm_body is None:
            return
        for stmt in chunk.vm_body.stmts:
            ...
            if isinstance(stmt, ast.While):
                self._recover_opcodes_from_block(stmt.body)

    def _recover_opcodes_from_block(self, body: ast.Block):
        for stmt in body.stmts:
            if isinstance(stmt, ast.If):
                for i, test in enumerate(stmt.tests):
                    if isinstance(test, ast.BinOp) and test.op == '==':
    ...
    def _build_prototype(self, chunk: PSUChunk) -> VMPrototype:
        proto = VMPrototype()
        ...
        for pc, inst_data in enumerate(chunk.instructions):
            if isinstance(inst_data, (list, tuple)) and len(inst_data) >= 1:
                proto.instructions.append(VMInstruction(
                    opcode=inst_data[0],
                    operands=list(inst_data[1:]),
                    pc=pc,
                ))
        return proto
    ...
    def _decode_inst(self, std_op: str, inst: VMInstruction, proto: VMPrototype) -> List[ast.Stmt]:
        ...
        if std_op == 'LOADK' and len(ops) >= 2:
            val = self._const_ast(proto, ops[1])
        ...
        if std_op == 'CLOSURE' and len(ops) >= 1:
            return [ast.LocalAssign(
                names=[f"v{ops[0]}"],
...
def deobfuscate_psu(source: str, sandbox: Optional[LuaSandbox] = None) -> Optional[ast.Block]:
    d = PSUDeobfuscator(sandbox)
